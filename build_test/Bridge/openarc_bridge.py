"""
Nahel OpenArc Bridge — Minimal FastAPI server wrapping openvino_genai.LLMPipeline.
Usage:
    python openarc_bridge.py --model_path <path> --model_name <name> --device CPU --port 8100
"""
import argparse
import asyncio
import json
...
app = FastAPI(title="Nahel OpenArc Bridge")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def main():
    parser = argparse.ArgumentParser(description="Nahel OpenArc Bridge")
    ...
    print(f"[OpenArc Bridge] Loading model from {model_path} on {args.device}...")

    ov_genai, AutoTokenizer = _load_openvino()
    _pipeline = ov_genai.LLMPipeline(str(model_path), args.device)
    _tokenizer = AutoTokenizer.from_pretrained(str(model_path))

    print(f"[OpenArc Bridge] Model '{args.model_name}' ready on port {args.port}.")
