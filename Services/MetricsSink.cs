...
public class MetricsSink
{
    ...
    public MetricsSink()
    {
        ...
        if (!File.Exists(_csvFile))
            File.WriteAllText(_csvFile, "timestamp,req_id,model,backend,device,ttft_ms,tok_s,ram_mb,duration_ms" + Environment.NewLine);
    }

    public void Record(string reqId, string model, string backend, string device, double ttft, double tokS, long ramMb, double durationMs)
    {
        var line = $"{DateTime.Now:O},{reqId},{model},{backend},{device},{ttft:F1},{tokS:F2},{ramMb},{durationMs:F1}";
