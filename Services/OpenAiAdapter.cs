...
{
    public Task<object> ToBackendFormatAsync(JsonElement ollamaRequest, string modelRealName, string backendType)
    {
        if (backendType == "embed")
        {
        ...
        if (ollamaRequest.TryGetProperty("images", out var imgs) && imgs.GetArrayLength() > 0)
        {
            ...
            foreach (var img in imgs.EnumerateArray())
            {
                ...
                {
                    type = "image_url",
                    image_url = new { url = $"data:image/jpeg;base64,{img.GetString()}" }
                });
            }
    ...
    public async IAsyncEnumerable<string> FromBackendStreamAsync(Stream backendStream, string modelName, CancellationToken ct)
    {
